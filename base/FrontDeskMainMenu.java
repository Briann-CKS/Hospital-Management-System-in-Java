...
public class FrontDeskMainMenu implements Serializable {
	...
	public FrontDeskMainMenu (FrontDesk f1, Vector d, Vector n, Vector f, Vector p, ArrayList sp)
	{
		...
		while (keep_going)
		{
			...
			{
				listPatient(p);
				System.out.println ("\nPlease select a patient to update his/her medical bill:");
				int num = gen.nextInt();
				if (num > p.size() || num < 1)
				{
			...
			else if (choice == 10)
			{
				...
				if (type == 1)
				{
					...
					while (kg)
					{
						...
						if (nspecial > sp.size() || nspecial < 1)
						{
